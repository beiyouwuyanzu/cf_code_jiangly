...
int main() {
  ...
  while (cas--) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (SZ(a) == 0 && mask == 0) continue;
      a.push_back(mask);
    }
    while (SZ(a) && a.back() == 0) a.pop_back();
    n = SZ(a);
    auto dp = vect<int>(-1, n, 4);
    ...
    function<int(int, int)> solve =
      [&](int pos, int mask) {
        ...
        for (int nmask = 0; nmask < 4; ++nmask) {
          if ((nmask & a[pos]) == a[pos] && (nmask & mask) != 0) {
            int cur = __builtin_popcount(nmask) + solve(pos + 1, nmask);
            ckmin(ret, cur);
          }
        }
        return ret;
