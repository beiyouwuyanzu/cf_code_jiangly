...
// time:2023-02-01 18:03:58
// name:C-FlexibleString
...
int main() {
  ...
  while (cas--) {
    ...
    cin >> s >> t;
    set<int> C;
    for (auto& c : s) C.insert(c);
    vector<char> a(C.begin(), C.end());
    int len = SZ(a);
    trace(a, len);
    int64 ret = 0;
    for (int mask = 0; mask < (1 << len); ++mask) {
      ...
      vector<bool> flag(26);
      for (int i = 0; i < len; ++i) {
        if ((mask >> i) & 1) flag[a[i] - 'a'] = 1;
      }
      int64 cur = 0;
      for (int i = 0, j; i < n; i = j) {
        for (; i < n && (s[i] != t[i] && !flag[s[i] - 'a']); ++i);
        if (i == n) break;
        for (j = i + 1; j < n && !(s[j] != t[j] && !flag[s[j] - 'a']); ++j);
        // trace(i, j);
        int64 L = j - i;
        cur += L * (L + 1) / 2;
      }
