// url:https://codeforces.com/contest/1788/problem/D
// time:2023-02-09 18:08:45
// name:D-MovingDots
...
int main() {
  ...
  vector<mint> p2(n + 1);
  for (int i = 0; i <= n; ++i) p2[i] = i == 0 ? 1 : p2[i - 1] * 2;
  vector<mint> sumL(n + 1), sumR(n + 1);
  for (int i = 0; i < n; ++i) {
    sumL[i + 1] = sumL[i] + p2[i];
    sumR[i + 1] = sumR[i] + p2[n - 1 - i];
  }
  ...
  for (int i = 1; i < n; ++i) {
    for (int j = i + 1; j < n - 1; ++j) {
      int D = a[j] - a[i];
      int rr = lower_bound(a.begin(), a.end(), a[j] + D) - a.begin();
      int ll = lower_bound(a.begin(), a.end(), a[i] - D) - a.begin();
      if (rr > j + 1 && ll < i) {
        ret += (sumR[rr] - sumR[j + 1]) * (sumL[i] - sumL[ll]);
      }
    }
  }
  ret += p2[n] - 1;
