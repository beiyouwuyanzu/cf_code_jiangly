...
int main() {
  ...
  vector<int64> pos;
  for (int i = 0; i <= n; ++i) pos.push_back(S[i]);
  dedup(pos);
  vector<int> id(n + 1);
  for (int i = 0; i <= n; ++i) {
    id[i] = lower_bound(pos.begin(), pos.end(), S[i]) - pos.begin() + 1;
  }

  Fenwick<int64> F(SZ(pos));
  vector<int64> dp(n + 1);
  F.add(id[0], dp[0] - 0);

  for (int i = 1; i <= n; ++i) {
    dp[i] = dp[i - 1];
    ckmax(dp[i], F.query(id[i]) + i);
    F.add(id[i], dp[i] - i);
  }
  cout << dp[n] << '\n';
  return 0;
