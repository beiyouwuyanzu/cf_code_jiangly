...
// time:2022-03-31 18:05:11
// name:F1-PromisingString(easyversion)
  ...
  while (cas--) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      for (int j = i; j < n; ++j) {
        ...
        int cur = cnt + max(0, len - 1);
        if (cntA <= cntB && (cntB - cntA) % 3 == 0 &&
            (cntB - cntA) / 3 <= cur) {
          ++ret;
          // trace(i, j, cntA, cntB, cur, ret);
        }
      }
    }
    cout << ret << '\n';
