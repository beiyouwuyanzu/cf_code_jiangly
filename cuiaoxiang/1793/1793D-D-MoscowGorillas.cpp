...
// time:2023-02-12 12:04:04
// name:D-MoscowGorillas
...
int main() {
  ...
  for (int i = 0; i < n; ++i) cin >> p[i], posP[p[i]] = i + 1;
  for (int i = 0; i < n; ++i) cin >> q[i], posQ[q[i]] = i + 1;

  int64 ret = 1;
  int64 L = posP[1], R = posQ[1];
  if (L > R) swap(L, R);
  ret += C2(L - 1) + C2(n - R) + C2(R - L - 1);
  trace(ret);
  for (int k = 2; k <= n; ++k) {
    int64 x = posP[k], y = posQ[k];
    if (x > y) swap(x, y);
    bool ok = 1;
    if (x >= L && x <= R) ok = 0;
    if (y >= L && y <= R) ok = 0;
